...
router.post("/", requireAdmin, validateBody(formSchema), (req: Request, res: Response) => {
  ...
  db.prepare(
    `INSERT INTO forms (id, name, description, version, fields, isActive, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
formSubmissionsRouter.get("/", requireStaff, (req, res) => {
  ...
  if (formId) {
    ...
    params.push(formId as string);
  }
  if (customerId) {
    conditions.push("customerId = ?");
    params.push(customerId as string);
  }
  if (dogId) {
    conditions.push("dogId = ?");
    params.push(dogId as string);
  }
  if (appointmentId) {
    conditions.push("appointmentId = ?");
    params.push(appointmentId as string);
  }
...
formSubmissionsRouter.post("/", validateBody(formSubmissionSchema), (req: Request, res: Response) => {
  ...
  db.prepare(
    `INSERT INTO form_submissions (id, formId, customerId, dogId, appointmentId, data, signature, submittedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
