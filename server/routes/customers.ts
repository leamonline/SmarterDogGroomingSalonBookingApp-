...
function hydrateCustomers(customerRows: CustomerRow[]) {
  ...
  const warnings = db
    .prepare(`SELECT customerId, warning FROM customer_warnings WHERE customerId IN (${placeholders})`)
    .all(...customerIds) as WarningRow[];
  const pets = db.prepare(`SELECT * FROM pets WHERE customerId IN (${placeholders})`).all(...customerIds) as (PetRow & {
    behavioralNotes?: string[];
    vaccinations?: VaccinationRow[];
  })[];
  const documents = db
    .prepare(`SELECT * FROM documents WHERE customerId IN (${placeholders})`)
    .all(...customerIds) as DocumentRow[];
  ...
  const behavioralNotes =
    petIds.length > 0
      ? (db
          .prepare(`SELECT petId, note FROM pet_behavioral_notes WHERE petId IN (${petPlaceholders})`)
          .all(...petIds) as BehavioralNoteRow[])
      : [];
  const vaccinations =
    petIds.length > 0
      ? (db
          .prepare(`SELECT * FROM vaccinations WHERE petId IN (${petPlaceholders})`)
          .all(...petIds) as VaccinationRow[])
      : [];
...
router.get("/appointment-lookup", (req, res) => {
  ...
  const noteRows =
    petIds.length > 0
      ? (db
          .prepare(`SELECT petId, note FROM pet_behavioral_notes WHERE petId IN (${petIds.map(() => "?").join(",")})`)
          .all(...petIds) as BehavioralNoteRow[])
      : [];
...
router.post("/", validateBody(customerSchema), (req: Request, res: Response) => {
  ...
  db.transaction(() => {
    db.prepare(
      `INSERT INTO customers (id, name, email, phone, address, emergencyContactName, emergencyContactPhone, notes, lastVisit, totalSpent) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      id,
      noUndef(name),
      noUndef(email),
      noUndef(phone),
      noUndef(address),
      noUndef(emergencyContact?.name),
      noUndef(emergencyContact?.phone),
      noUndef(notes),
      noUndef(lastVisit),
      noUndef(totalSpent),
    );
    ...
    const insertPet = db.prepare(
      "INSERT INTO pets (id, customerId, name, breed, weight, dob, coatType) VALUES (?, ?, ?, ?, ?, ?, ?)",
    );
    ...
    savedPets.forEach((p: PetInput) => {
      ...
      (p.vaccinations || []).forEach((v: VaxInput) =>
        insertVax.run(petId, noUndef(v.name), noUndef(v.expiryDate), noUndef(v.status)),
      );
    });
    ...
    (documents || []).forEach((d: DocInput) =>
      insertDoc.run(crypto.randomUUID(), id, noUndef(d.name), noUndef(d.type), noUndef(d.uploadDate), noUndef(d.url)),
    );
  })();
...
router.put("/:id", validateBody(customerSchema), (req: Request, res: Response) => {
  ...
  db.transaction(() => {
    ...
    if (incomingPetIds.length > 0) {
      ...
      db.prepare(`DELETE FROM pets WHERE customerId = ? AND id NOT IN (${placeholders})`).run(
        customerId,
        ...incomingPetIds,
      );
    } else {
    ...
    const upsertPet = db.prepare(
      `INSERT INTO pets (id, customerId, name, breed, weight, dob, coatType) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET name=excluded.name, breed=excluded.breed, weight=excluded.weight, dob=excluded.dob, coatType=excluded.coatType`,
    );
    ...
    (pets || []).forEach((p: PetInput) => {
      ...
      (p.vaccinations || []).forEach((v: VaxInput) =>
        insertVax.run(noUndef(p.id), noUndef(v.name), noUndef(v.expiryDate), noUndef(v.status)),
      );
    });
