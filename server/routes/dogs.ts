...
function hydrateDogs(dogs: DogListRow[]) {
  ...
  const behavioralNotes = db
    .prepare(`SELECT petId, note FROM pet_behavioral_notes WHERE petId IN (${placeholders})`)
    .all(...dogIds) as BehavioralNoteRow[];
  const vaccinations = db
    .prepare(`SELECT * FROM vaccinations WHERE petId IN (${placeholders})`)
    .all(...dogIds) as VaccinationRow[];
  const tags = db
    .prepare(`SELECT dogId, tag FROM dog_tags WHERE dogId IN (${placeholders})`)
    .all(...dogIds) as TagRow[];
